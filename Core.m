...
for t = 1:params.Ntime
    ...
    % Update potential and field (using GPU-based density estimation)
    ...
    % Update particle positions using the unified GPU loop (sign=-1 for electrons, +1 for holes)
    ...
    % Remove out‐of‐bound (NaN) particles
    ...
    % Inject new particles at the left/right boundaries
    ...
    % Update observables (currents, velocities, densities, etc.)
...
function params = initParameters()
    % Physical constants
    params.eps_0 = 8.854187818e-12;
    params.m_0   = 9.109534e-31;
    params.hbar  = 1.0545887e-34;
    params.h     = 2*pi*params.hbar;
    params.kb    = 1.380662e-23;
    params.q     = 1.6021892e-19;
    ...
    % Time steps and scattering parameters
...
function [particles, energyStruct] = initParticles(params)
    % Initialize carrier positions and k-vectors (normalize inline)
    ...
    % Sample energy partitions using provided distributions
...
function [r_new, k_new, into_Cu, into_ITO] = updateParticles_gpu(type, r, k, params, gpu_Efield, gpu_rand, scatter)
    ...
    % Extract constant scalars
    del_t_eh = params.del_t_eh;
    device_thickness = params.device_thickness;
    
    % Call arrayfun directly with the function handle to carrierLoopV1
...
% Unified carrier loop for both electrons (sign=-1) and holes (sign=+1)
function [r, kx, ky, kz, into_Cu, into_ITO] = carrierLoopV1(r_val, kx, ky, kz, m_val, dt, scatter, device_thickness, Efield_int, r_rand, signVal)
    ...
    if dt * scatter > r_rand  % collision event
        vecnorm = sqrt(kx^2 + ky^2 + kz^2);
        % Generate three independent random numbers (avoiding vector indexing)
        r1 = 2*rand() - 1;
        r2 = 2*rand() - 1;
        r3 = 2*rand() - 1;
        rnd_norm = sqrt(r1^2 + r2^2 + r3^2);
        if rnd_norm == 0
            rnd_norm = 1;  % avoid division by zero
        end
        r1 = r1 / rnd_norm;
        r2 = r2 / rnd_norm;
        r3 = r3 / rnd_norm;
    ...
    % Remove carriers that exit the device boundaries
    if r > device_thickness
         r = NaN; kx = NaN; ky = NaN; kz = NaN; 
         into_Cu = 1;
    elseif r < 0
         r = NaN; kx = NaN; ky = NaN; kz = NaN; 
         into_ITO = 1;
...
function [r, k, injLeft, injRight] = injectParticles(r, k, params, Epart, type)
    ...
    desired = (strcmp(type,'electron'))*params.N_e/params.npoints + (strcmp(type,'hole'))*params.N_h/params.npoints;
...
function [r, k] = injectBoundary(r, k, injCount, Epart, params, type, side)
    ...
    for i = 1:injCount
        ...
        k_new(i,:) = k_new(i,:) * params.knorm(Epart(idx(i))-params.Egap, m_eff);
    end
    k = [k; k_new];
    if strcmp(side, 'left')
...
function observables = updateObservables(obs, V, into, injLeft, injRight, particles, params, t)
...
function plotPostProcessing(obs, params)
    ...
    figure;
    plot(t_vec, smooth(smooth(obs.J)), t_vec, obs.J);
    grid on;
    title('Current vs Time');
    xlabel('Time (s)'); ylabel('Current (A/m^2)');
    
    figure;
    plot(t_vec, sum(obs.elec_vs_t,1), t_vec, sum(obs.hole_vs_t,1));
    grid on;
    ...
    figure;
    mesh(obs.hole_vs_t); view(2);
    title('Hole Particle Density vs Time');
    xlabel('Time step'); ylabel('Position (Angstrom)'); colorbar;
    
    figure;
    mesh(obs.elec_vs_t); view(2);
...
function [qFn, qFp] = quasifermifind(e_density,h_density,m_e,m_h,Egap)
    ...
    h = 2*pi*hbar;
    F= @(x,c) x.^(1/2)./(1+exp(x-c));
    itmax = 5000; tol = Egap/1e17; itr=1;
    ea=0; eb = Egap;
